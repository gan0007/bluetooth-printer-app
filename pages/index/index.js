...
function bleDoWrite(value) {
  wx.writeBLECharacteristicValue({
    deviceId: connectInfo._deviceId, serviceId: connectInfo._serviceId,
    characteristicId: connectInfo._characteristicId, value: value
  });
}

function writeText(str) {
  var bytes = GBK.encode(str);
...
function writeBuffer(buffer) { bleDoWrite(buffer); }

var PACK_SIZE = 100;
function writeBufferPack(buffer) {
  return new Promise(function(resolve, reject) {
    function send(pos) {
      if (pos >= buffer.byteLength) { resolve('OK'); return; }
      var end = Math.min(pos + PACK_SIZE, buffer.byteLength);
      wx.writeBLECharacteristicValue({
        deviceId: connectInfo._deviceId, serviceId: connectInfo._serviceId,
        characteristicId: connectInfo._characteristicId, value: buffer.slice(pos, end),
        success: function() { send(end); }, fail: reject
      });
    }
    send(0);
  });
...
function isConnected() { return connectInfo.canWrite && connectInfo._deviceId !== ''; }

function resetConnection(deviceId) {
  wx.closeBLEConnection({ deviceId: deviceId });
  connectInfo.canWrite = false; connectInfo._deviceId = ''; connectInfo._serviceId = ''; connectInfo._characteristicId = '';
}

function startConnectionMonitor(deviceId) {
  wx.offBLEConnectionStateChange();
  wx.onBLEConnectionStateChange(function(res) {
    if (!res.connected && res.deviceId === connectInfo._deviceId) {
      connectInfo.canWrite = false; connectInfo._deviceId = ''; connectInfo._serviceId = ''; connectInfo._characteristicId = '';
    }
  });
}
  ...
  wx.getBLEDeviceServices({
    ...
    success: function(res) {
      for (var i = 0; i < res.services.length; i++) {
        _getCharacteristics(deviceId, res.services[i].uuid);
      }
    }
  });
}
    ...
    success: function(res) {
      for (var i = 0; i < res.characteristics.length; i++) {
        ...
        if (item.properties.write) {
          connectInfo.canWrite = true;
          connectInfo._deviceId = deviceId;
          connectInfo._serviceId = serviceId;
          connectInfo._characteristicId = item.uuid;
          wx.showToast({ title: '连接成功', icon: 'success', duration: 1500 });
        }
        if (item.properties.notify || item.properties.indicate) {
          wx.notifyBLECharacteristicValueChange({ deviceId: deviceId, serviceId: serviceId, characteristicId: item.uuid, state: true });
        }
      }
    }
  });
  wx.onBLECharacteristicValueChange(function(c) {
    var v = ab2hex(c.value);
    connectInfo.status = ({ '12': '正常', '16': '纸仓盖开', '56': '纸仓盖开', '32': '缺纸' })[v] || '正常';
  });
}
...
function processGraphic(img, tw, th) {
  var cw = Math.min(tw || img.width, MAX_DIM);
  var ch = Math.min(th || img.height, MAX_DIM);
  return new Promise(function(resolve, reject) {
    ...
    image.onload = function() {
      ...
      var hex = convertToHex(imgData);
      var dataBuf = new Uint8Array(hex.data.buffer);
      var hexW = Math.floor(hex.width);
      var totalH = hex.height, startH = 0;
      var maxRowsPerChunk = 128;
      var dataList = [];
      while (totalH > 0) {
        var sendH = Math.min(maxRowsPerChunk, totalH);
        var block = dataBuf.slice(startH * hexW, (startH + sendH) * hexW);
        // 使用 1D 76 30 不压缩打印 (Raster Bitmap)
        // GS v 0 m xL xH yL yH d1...dk
        var header = makeESC(0x1D, 0x76, 0x30, 0x00, hexW % 256, Math.floor(hexW / 256), sendH % 256, Math.floor(sendH / 256));
        dataList.push(mergeBufs([header, block]));
        startH += sendH; totalH -= sendH;
      }
      resolve(mergeBufs(dataList));
    };
...
function convertToHex(img) {
  ...
  var hexW = Math.ceil(w / 8);
  var hexData = new Uint8ClampedArray(hexW * h);
  var temp = 0, x = 0;
  w *= 4;
  for (var hi = 0; hi < h; hi++) {
    for (var wi = 0, k = 0; wi < w; wi++, k++) {
      var g = data[hi * w + wi] * 0.3 + data[hi * w + wi + 1] * 0.59 + data[hi * w + wi + 2] * 0.11;
      wi += 3;
      var j = k % 8;
      if (k != 0 && j == 0) { hexData[x] = temp; temp = 0; x++; }
      if (g < 128) temp |= (1 << (7 - j));
    }
    hexData[x] = temp; temp = 0; x++;
  }
  return { width: hexW, height: h, data: hexData };
}

function mergeBufs(buffers) {
  var total = 0;
  for (var i = 0; i < buffers.length; i++) total += buffers[i].byteLength;
  var merged = new ArrayBuffer(total);
  var target = new Uint8Array(merged);
  var offset = 0;
  for (var i = 0; i < buffers.length; i++) { target.set(new Uint8Array(buffers[i]), offset); offset += buffers[i].byteLength; }
  return merged;
}
...
// ===== App 初始化 =====
App({
  globalData: { deviceId: '', deviceName: '', blocks: [] }
});

// ===== 页面逻辑 =====
Page({
  data: {
    blocks: [], modal: null, editingIndex: -1,
    editData: { type: 'text', content: '', fontSize: 0, bold: false, align: 0, imagePath: '', codeContent: '' },
    typeLabels: { text: '文字', image: '图片', qrcode: '二维码', barcode: '条形码', divider: '分割线', blank: '空行', feed: '走纸' },
    connected: false, deviceName: '', deviceId: '',
    statusText: '等待连接', statusDesc: '尚未连接蓝牙打印机',
    editorMode: false, connMode: false,
    prompt: '初始化蓝牙...', promptType: 'info',
    searching: false, devices: [], btSearch: '开始搜索', pulseActive: false
  },
  ...
  onShow: function() { this.refreshStatus(); },

  refreshStatus: function() {
    var app = getApp();
    var conn = isConnected();
    this.setData({
      connected: conn, deviceName: app.globalData.deviceName, deviceId: app.globalData.deviceId,
      statusText: conn ? '打印机就绪' : '等待连接',
      statusDesc: conn ? '已连接 · 可随时打印' : '尚未连接蓝牙打印机'
    });
  },

  // 页面切换
  goConnect: function() { this.setData({ connMode: true }); this.openBluetooth(); },
  goHome: function() {
    if (this.data.searching) this.stopSearch();
    this.setData({ connMode: false, editorMode: false }); this.refreshStatus();
  },
  goEditor: function() {
    if (!isConnected()) {
      var that = this;
      wx.showModal({ title: '未连接打印机', content: '需要先连接蓝牙打印机', confirmText: '去连接',
        success: function(res) { if (res.confirm) that.setData({ connMode: true }); } });
      return;
    }
    this.setData({ editorMode: true });
  },

  disconnect: function() {
    var that = this;
    wx.showModal({ title: '断开连接', content: '确定断开蓝牙连接？',
      success: function(res) {
        if (res.confirm) {
          resetConnection(getApp().globalData.deviceId);
          getApp().globalData.deviceId = ''; getApp().globalData.deviceName = '';
          that.refreshStatus();
        }
      } });
  },
  ...
  openBluetooth: function() {
    ...
    wx.openBluetoothAdapter({
      success: function() { that.setData({ prompt: '蓝牙已就绪，搜索周围的打印机', promptType: 'ok' }); },
      fail: function(res) {
        if (res.errCode === 10001) {
          that.setData({ prompt: '请开启手机蓝牙', promptType: 'warn' });
          that._adapterStateHandler = function(r) { if (r.available) that.setData({ prompt: '蓝牙已开启', promptType: 'ok' }); };
          wx.onBluetoothAdapterStateChange(that._adapterStateHandler);
        } else { that.setData({ prompt: '蓝牙初始化失败', promptType: 'err' }); }
      }
    });
  },

  toggleSearch: function() { this.data.searching ? this.stopSearch() : this.startSearch(); },

  startSearch: function() {
    var that = this;
    this.setData({ searching: true, btSearch: '停止搜索', devices: [], prompt: '正在扫描...', promptType: 'ok', pulseActive: true });
    wx.startBluetoothDevicesDiscovery({
      allowDuplicatesKey: true, interval: 1000,
      success: function() {
        that._deviceFoundHandler = function(res) {
          var found = that.data.devices.slice();
          var changed = false;
          res.devices.forEach(function(d) {
            if (!d.name && !d.localName) return;
            var idx = found.findIndex(function(x) { return x.deviceId === d.deviceId; });
            if (idx === -1) { found.push(d); changed = true; } else { found[idx] = d; }
          });
          if (!changed) return;
          found.sort(function(a, b) { return b.RSSI - a.RSSI; });
          that.setData({ devices: found });
        };
        wx.onBluetoothDeviceFound(that._deviceFoundHandler);
      },
      fail: function() { that.setData({ searching: false, btSearch: '开始搜索', prompt: '搜索失败', promptType: 'err', pulseActive: false }); }
    });
  },

  stopSearch: function() {
    wx.stopBluetoothDevicesDiscovery();
    if (this._deviceFoundHandler) { wx.offBluetoothDeviceFound(this._deviceFoundHandler); this._deviceFoundHandler = null; }
    this.setData({ searching: false, btSearch: '开始搜索', prompt: '已停止', promptType: 'info', pulseActive: false });
  },

  connectDevice: function(e) {
    var that = this;
    var deviceid = e.currentTarget.dataset.deviceid;
    var name = e.currentTarget.dataset.name || '未知设备';
    if (this.data.searching) this.stopSearch();
    this.setData({ prompt: '正在连接 ' + name + ' ...', promptType: 'ok' });
    wx.showLoading({ title: '连接中...', mask: true });
    wx.createBLEConnection({
      deviceId: deviceid,
      success: function() {
        wx.setBLEMTU({ deviceId: deviceid, mtu: 100, success: function() {} });
        getApp().globalData.deviceId = deviceid;
        getApp().globalData.deviceName = name;
        getBLEDeviceServices(deviceid);
        startConnectionMonitor(deviceid);
        wx.hideLoading();
        wx.showToast({ title: '连接成功', icon: 'success' });
        setTimeout(function() { that.goHome(); }, 1200);
      },
      fail: function(err) {
        wx.hideLoading();
        var msg = err.errCode === 10003 ? '设备无响应，请确认打印机已开机' : err.errCode === 10012 ? '连接超时，靠近设备后重试' : '连接失败';
        that.setData({ prompt: msg, promptType: 'err' });
  ...
  // 编辑器
  noop: function() {},
  showAddPanel: function() { this.setData({ modal: 'add' }); },
  hideAddPanel: function() { this.setData({ modal: null }); },

  addBlock: function(e) {
    var type = e.currentTarget.dataset.type;
    this.setData({ modal: null });
    var block = this.createBlock(type);
    var blocks = this.data.blocks.concat([block]);
    this.setData({ blocks: blocks });
    this.saveBlocks();
    var idx = blocks.length - 1;
    if (type === 'text') this.editBlockIdx(idx);
    else if (type === 'image') this.pickImage(idx);
    else if (type === 'qrcode' || type === 'barcode') this.editBlockIdx(idx);
  },

  createBlock: function(type) {
    var base = { type: type, id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6) };
    switch (type) {
      case 'text': return Object.assign({}, base, { content: '双击编辑文字', fontSize: 0, bold: false, align: 0 });
      case 'image': return Object.assign({}, base, { imagePath: '', imageWidth: 200, imageHeight: 200 });
      case 'qrcode': return Object.assign({}, base, { content: 'https://www.example.com' });
      case 'barcode': return Object.assign({}, base, { content: '1234567890' });
      case 'divider': return Object.assign({}, base, { content: '-------------------------------' });
      case 'blank': return Object.assign({}, base, { content: '' });
      case 'feed': return Object.assign({}, base, { content: '', feedLines: 4 });
      default: return base;
    }
  },
  ...
  editBlock: function(e) {
    ...
    if (!block) return;
    // 图片块直接打开选择器，不弹编辑窗
    if (block.type === 'image') { this.pickImage(index); return; }
    // 文字、二维码、条形码弹出编辑窗
    this.setData({
      modal: 'edit', editingIndex: index,
      editData: { type: block.type, content: block.content || '', fontSize: block.fontSize || 0,
        bold: block.bold || false, align: block.align || 0, imagePath: block.imagePath || '', codeContent: block.content || '' }
    });
  },

  editBlockIdx: function(index) {
    var block = this.data.blocks[index];
    if (!block) return;
    this.setData({
      modal: 'edit', editingIndex: index,
  ...
  onEditContentInput: function(e) { this.setData({ 'editData.content': e.detail.value }); },
  onCodeContentInput: function(e) { this.setData({ 'editData.codeContent': e.detail.value }); },
  toggleBold: function() { this.setData({ 'editData.bold': !this.data.editData.bold }); },
  setFontSize: function(e) { this.setData({ 'editData.fontSize': parseInt(e.currentTarget.dataset.size) }); },
  setAlign: function(e) { this.setData({ 'editData.align': parseInt(e.currentTarget.dataset.align) }); },

  confirmEdit: function() {
    var block = this.data.blocks[this.data.editingIndex];
    var ed = this.data.editData;
    if (block.type === 'text') { block.content = ed.content || ' '; block.fontSize = ed.fontSize; block.bold = ed.bold; block.align = ed.align; }
    else if (block.type === 'qrcode' || block.type === 'barcode') block.content = ed.codeContent || block.content;
    this.setData({ blocks: this.data.blocks, modal: null });
    this.saveBlocks();
  },

  cancelEdit: function() { this.setData({ modal: null }); },
      ...
      success: function(res) {
        wx.getImageInfo({
          ...
          success: function(info) {
            ...
            blocks[index].imageWidth = w; blocks[index].imageHeight = h;
            that.setData({ blocks: blocks }); that.saveBlocks();
          }
        });
      }
    });
  ...
  moveBlockUp: function(e) {
    ...
    if (i <= 0) return;
    var blocks = this.data.blocks.slice();
    var tmp = blocks[i]; blocks[i] = blocks[i - 1]; blocks[i - 1] = tmp;
    this.setData({ blocks: blocks }); this.saveBlocks();
  },

  moveBlockDown: function(e) {
    var i = parseInt(e.currentTarget.dataset.index);
    if (i >= this.data.blocks.length - 1) return;
    var blocks = this.data.blocks.slice();
    var tmp = blocks[i]; blocks[i] = blocks[i + 1]; blocks[i + 1] = tmp;
    this.setData({ blocks: blocks }); this.saveBlocks();
  },
  ...
  deleteBlock: function(e) {
    var that = this;
    var i = parseInt(e.currentTarget.dataset.index);
    wx.showModal({ title: '确认删除', content: '确定要删除这个内容块吗？',
      success: function(res) {
        if (res.confirm) {
          var blocks = that.data.blocks.slice(); blocks.splice(i, 1);
          that.setData({ blocks: blocks }); that.saveBlocks();
        }
      } });
  },
  ...
  saveBlocks: function() {
    ...
    this._saveTimer = setTimeout(function() { wx.setStorage({ key: 'editor_blocks', data: that.data.blocks }); that._saveTimer = null; }, 300);
  },

  printAll: function() {
    if (!isConnected()) {
      var that = this;
      wx.showModal({ title: '未连接', content: '请先连接蓝牙打印机', confirmText: '去连接',
        success: function(res) { if (res.confirm) that.setData({ connMode: true }); } });
      return;
    }
    var blocks = this.data.blocks;
    if (blocks.length === 0) { wx.showToast({ title: '请先添加内容', icon: 'none' }); return; }
    wx.showLoading({ title: '正在打印...', mask: true });

    var that = this;
    var lf = { fontSize: -1, bold: null, align: -1 };

    function run() {
      writeBuffer(makeESC(0x1B, 0x40));
      return delay(50).then(function() {
        function next(i) {
          ...
          return that._printBlock(blocks[i], lf).then(function() { return delay(80); }).then(function() { return next(i + 1); });
        }
        return next(0);
      }).catch(function(e) {
        wx.hideLoading(); wx.showToast({ title: '打印失败', icon: 'none' });
      });
    }
    run();
  },

  _printBlock: function(block, lf) {
    switch (block.type) {
      case 'text':
        var content = block.content || ' ';
        var sb = 0x00;
        if (block.fontSize === 1) sb = 0x10; else if (block.fontSize === 2) sb = 0x01; else if (block.fontSize === 3) sb = 0x11;
        if (lf.fontSize !== block.fontSize) { writeBuffer(makeESC(0x1D, 0x21, sb)); lf.fontSize = block.fontSize; }
        if (lf.bold !== block.bold) { writeBuffer(makeESC(0x1B, 0x45, block.bold ? 0x01 : 0x00)); lf.bold = block.bold; }
        if (lf.align !== block.align) { writeBuffer(makeESC(0x1B, 0x61, block.align)); lf.align = block.align; }
        writeText(content + '\r\n');
        return Promise.resolve();
      case 'image':
        if (!block.imagePath) return Promise.resolve();
        var that = this;
        return new Promise(function(resolve, reject) {
          wx.getImageInfo({
            src: block.imagePath,
      ...
      case 'qrcode':
        ...
        writeBuffer(makeESC.apply(null, bytes));
        writeBuffer(makeESC(0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x51, 0x30));
        writeText('\r\n');
        return Promise.resolve();
      case 'barcode':
        ...
        for (var j = 0; j < bc.length; j++) bbytes.push(bc.charCodeAt(j));
        writeBuffer(makeESC.apply(null, bbytes));
        writeText('\r\n');
        return Promise.resolve();
      case 'divider':
      ...
      case 'blank':
        ...
        return Promise.resolve();
      case 'feed':
        writeBuffer(makeESC(0x1B, 0x64, block.feedLines || 4));
        return Promise.resolve();
      default:
        return Promise.resolve();
    }
  ...
  clearAll: function() {
    ...
    wx.showModal({ title: '确认清空', content: '确定要清空所有内容吗？',
      success: function(res) { if (res.confirm) { that.setData({ blocks: [] }); wx.removeStorageSync('editor_blocks'); } } });
  }
});
